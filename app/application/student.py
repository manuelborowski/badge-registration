from app.data import settings as msettings, student as mstudent, photo as mphoto
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if sdh_students['status']:
                ...
                db_students = mstudent.student_get_m()
                photo_ids = ",".join([str(s.foto_id) for s in db_students])
                sdh_photo_size_url = msettings.get_configuration_setting('sdh-photo-size-url')
                res = requests.get(sdh_photo_size_url, params = {"ids": photo_ids}, headers={'x-api-key': sdh_key})
                if res.status_code == 200:
                    photos_to_check = []
                    ...
                    if sdh_photo_data["status"]:
                        ...
                        if photos_to_check:
                            ...
                            photo_ids = ",".join([str(p) for p in photos_to_check])
                            res = requests.get(sdh_photo_url, params={"ids": photo_ids}, headers={'x-api-key': sdh_key})
                            if res.status_code == 200:
                                sdh_photo_data = res.json()
                                if sdh_photo_data["status"]:
                                    update_photos = []
